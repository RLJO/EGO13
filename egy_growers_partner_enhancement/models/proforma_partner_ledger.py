...
class ReportPartnerLedger(models.AbstractModel):
    ...
    _name = "account.proforma.partner.ledger"
    ...
    def _do_query_group_by_account(self, options, line_id):
        ...
        if query_res:
            ...
            return {line_id: {'partner_id': line_id, 'debit': 0.0, 'credit': 0.0, 'balance': 0.0}}
    ...
    def _group_by_partner_id(self, options, line_id):
        ...
        # if not results:
        #     results = {1954: {'partner_id': 1954, 'debit': 2000.0, 'credit': 2000.0, 'balance': 0.0}}
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for partner in sorted_partners:
            ...
            if 'partner_' + str(partner.id) in options.get('unfolded_lines') or unfold_all:
                ...
                lines += domain_lines
            #######################
            partner_invoice_proforma_state = self.env['account.invoice'].search(
                [('partner_id', '=', partner.id), ('state', '=', 'proforma')])
            for record in partner_invoice_proforma_state:
                init_balance = 0.0
                if len(lines) > 1:
                    if lines[-1].get('columns')[8].get('name'):
                        init_balance_currency = lines[-1].get('columns')[8].get('name')
                        # init_balance = Decimal(init_balance_currency.strip('$'))
                        init_balance = price_str(init_balance_currency)
                        total = float(init_balance) + record.amount_total
                    else:
                        total = record.amount_total
                    inv_list = [
                        {'name': 'INV'},
                        {'name': record.account_id.code},
                        {'name': 'Proforma Invoice'},
                        {'name': format_date(self.env, record.date_due), 'class': 'date'},
                        {'name': ''},
                        {'name': self.format_value(float(init_balance))},
                        {'name': self.format_value(record.amount_total)},
                        {'name': ''},
                    ...
                    lines.append({
                        'id': record.id,
                        'parent_id': 'partner_%s' % partner.id,
                        'name': format_date(self.env, record.date_invoice),
                        'class': 'date',
                        'columns': inv_list,
                        'caret_options': '',
                        'level': 4,
    ...
    def _get_report_name(self):
        return _('Proforma Partner Ledger')
