...
class EgyGrowersAccountInvoiceRefund(models.TransientModel):

    _inherit = 'account.invoice.refund'

    @api.model
    def _get_sale_order_tracking_no(self):
        context = dict(self._context or {})
        active_id = context.get('active_id', False)
        if active_id:
            inv = self.env['account.invoice'].browse(active_id)
            return inv.tracking_number
        return ''
    ...
    sale_order_id = fields.Char(default=_get_sale_order)
    tracking_number = fields.Char(string='Tracking No.', default=_get_sale_order_tracking_no)
    ...
    def compute_refund(self, mode='refund'):
        ...
        for form in self:
            ...
            for inv in inv_obj.browse(context.get('active_ids')):
                ...
                refund.write({'sale_order_id': inv.sale_order_id.id})
